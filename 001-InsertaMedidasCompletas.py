...
def insert_data(cursor, data):
    try:
        cursor.execute(
            """INSERT INTO produccion (guid, vin, medida, valor, unidad, StartDate, CargaAlta, CargaMedia, CargaBaja, Ralenti, KeyOn, latitud, longitud, org_nombre)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (
                ...
                data.get("Ralenti"), data.get("KeyOn"), data.get("latitud"), data.get("longitud"), data.get("org_nombre")
            )
        )
    except Exception as e:
...
if result:
    for org in result.get("values", []):
        org_nombre = "Nombre Desconocido"
        for link in org.get('links', []):
            if link['rel'] == 'machines' or  link['rel'] == 'organizations':
                machinesUri = link['uri']
                # Hacer la solicitud a la URI de las máquinas para obtener el nombre de la organización
                respuesta_machines = requests.get(machinesUri, headers=headers)
                if respuesta_machines.status_code == 200:
                    resultado_machines = respuesta_machines.json()
                    org_nombre = resultado_machines.get('name', "Nombre Desconocido")  # Obtener el nombre de la organización
                    #print (org_nombre)    
                ...
                if respuesta.status_code == 200:
                    ...
                    for hr in resultado.get('values', []):
                        if hr['machineMeasurementDefinition']['id'].upper() in ids_to_check:
                            ...
                            data_dict["org_nombre"] = org_nombre # Añadir el nombre de la organización
