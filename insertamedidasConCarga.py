...
def insert_data(cursor, data):
    try:
        cursor.execute(
            """INSERT INTO produccion (guid, vin, medida, valor, unidad, StartDate, CargaAlta, CargaMedia, CargaBaja, Ralenti, KeyOn)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
            (
                data["guid"], data["vin"], data["Medida"], data.get("valor"), data["unidad"],
                data.get("StartDate"), data.get("CargaAlta"), data.get("CargaMedia"), data.get("CargaBaja"),
                data.get("Ralenti"), data.get("KeyOn")
            )
        )
    except Exception as e:
